...
if [ "$cuda_major_version" == "V12" ]; then
    python3.9 -m pip install -U "jax[cuda12_pip]" -f https://storage.googleapis.com/jax-releases/jax_cuda_releases.html
elif [ "$cuda_major_version" == "V11" ]; then
    python3.9 -m pip install --upgrade "jax[cuda11_pip]" -f https://storage.googleapis.com/jax-releases/jax_cuda_releases.html
else
