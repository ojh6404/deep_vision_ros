...
class TrackAllNode(ConnectionBasedTransport):
    ...
    def callback(self, img_msg):
        ...
        if self.template_mask is not None:  # track start
            ...
            detic_msg = rospy.wait_for_message("~input_mask", SegmentationInfo)
            self.template_mask = self.bridge.imgmsg_to_cv2(detic_msg.segmentation, desired_encoding="32SC1").astype(np.uint8)
            self.masks = self.decompose_mask(self.template_mask)
